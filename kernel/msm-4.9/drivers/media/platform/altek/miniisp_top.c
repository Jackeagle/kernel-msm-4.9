...
int mini_isp_get_chip_id(u32 mini_isp_reg_addr, u8 *id_buf)
{
	...
	mini_isp_register_read(CHIP_ID_ADDR, (u32 *)&buff_id[0]);

	misp_err("[miniISP]Get Chip ID %x %x %x %x",
				buff_id[0], buff_id[1], buff_id[2], buff_id[3]);
...
u32 mini_isp_register_read(u32 reg_addr, u32 *reg_value)
{
	...
	if(SPI_SHORT_LEN_MODE)
	{
	    ...
	    status = devdata->intf_fn->read(
						    devdata,
						    send_buffer_value,
						    1,  // TX: cmd{ ctrlbyte[1byte] }
						    recv_buffer_value,
						    5); // RX: data{ dummy[1byte] + data[4byte] }
	...
	else
	{
		...
		if(dev_global_variable->intf_status & INTF_SPI_READY) {
			...
			if (EMODE_TXCMD_LEN + rx_dummy_len + 4 > 32)
				status = mini_isp_check_rx_dummy(&recv_buffer, rx_dummy_len+EMODE_TXCMD_LEN);
			else
				status = mini_isp_check_rx_dummy(&recv_buffer, rx_dummy_len);
	...
	char *dest_path, char *file_name)
{
	...
	/*write the file*/
	vfs_write(f, (char *)dump_memory, ouput_size, &f->f_pos);
 ...
 *\return
 */
u32 mini_isp_woi_memory_read_then_write_file(u32 start_addr, u32 lineoffset,
	u32 width, u32 height, char *dest_path, char *file_name)
{
	...
	io_size = EMODE_TXCMD_LEN*2 + rx_dummy_len + 60000;

	io_buffer = kzalloc(io_size, GFP_KERNEL);
	if (!io_buffer) {
		misp_err("%s Allocate memory failed.", __func__);
		goto allocate_io_memory_fail;
	}
	    ...
	    remain_size -= width, loopidx++)
	{
		...
		if(dev_global_variable->intf_status & INTF_SPI_READY) {
			...
			if (EMODE_TXCMD_LEN + rx_dummy_len + one_size > 32)
				retval = mini_isp_check_rx_dummy(&recv_buffer, rx_dummy_len+EMODE_TXCMD_LEN);
			else
				retval = mini_isp_check_rx_dummy(&recv_buffer, rx_dummy_len);
...
u32 mini_isp_memory_read(u32 start_addr, u8 *read_buffer, u32 len)
{
	...
	if(SPI_SHORT_LEN_MODE)
	{
		while (start_mem_addr < start_addr + len) {
			 ...
			 mini_isp_memory_read_shortlen( start_mem_addr, (u32 *)dump_memory );
	...
	else
	{
		...
		/* read 60000 bytes at a time;*/
		io_size = EMODE_TXCMD_LEN + rx_dummy_len + 60000;
		io_buffer = kzalloc(io_size, GFP_KERNEL);
		if (!io_buffer) {
			misp_err("%s Allocate memory failed.", __func__);
			goto allocate_memory_fail;
...
extern irqreturn_t mini_isp_irq(int irq, void *handle)
{
	...
	if (!devdata || !dev_global_variable)
		return -IRQ_NONE;
	if(dev_global_variable->AL6100_irq_flag == 0) {
		dev_global_variable->AL6100_irq_flag = 1;
