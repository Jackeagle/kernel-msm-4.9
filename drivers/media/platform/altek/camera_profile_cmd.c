						...
						u16 opcode, u8 *param)
{
	...
	misp_info("[led_onoff]: %d, [led_lv]: %d, [led_id]: %d, "
		"[delay_after_sof]: %d,	[pulse_time]: %d, [control_mode]: %d, "
		"[pulse_mode_skip_frame]: %d, [rolling_shutter]: %d",
		...
		led_power_control_param->pulse_mode_skip_frame,
						...
						u16 opcode, u8 *param)
{
	...
	misp_info("[deviceId]: %d, [delay_frame]: %d, [active_frame]: %d,"
		"[deactive_frame]: %d, [active_timeLv]: %d",
						...
						u16 opcode, u8 *param)
{
	...
	for (i = 0; i < lighting_ctrl_param->cycle_len; i++) {
		misp_info("cycle[%d]: 0x%x, 0x%x, %d",
			i, lighting_ctrl_param->cycle[i].source,
						...
						u16 opcode, u8 *param)
{
	...
	misp_info("%s - enter", __func__);
	misp_info("[en_updated]: 0x%x, [short_distance]: %d,"
		"[compensation]: %d",
						...
						u16 opcode, u8 *param)
{
	...
	errcode err = ERR_SUCCESS;
	u32 para_size;
	struct isp_cmd_cycle_trigger_depth_process *cycle_trigger_depth_process_param;
