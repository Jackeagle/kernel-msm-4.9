				...
				u16 opcode, u8 *param)
{
	...
	} else if (opcode == ISPCMD_BULK_WRITE_BOOTCODE_SHORT_LEN) { // Send Boot code short SPI len
...
#ifndef AL6100_SPI_NOR
		err = mast_bulk_data_cmd_write_boot_code_shortlen(devdata, param,
				g_filp[BOOT_CODE]);
#endif
	...
	} else if (opcode == ISPCMD_BULK_WRITE_BASICCODE_CODESUM) {  // Send Main code short SPI len
		/* Write Basic Code (short SPI len)*/
		misp_info("%s : write BASIC_CODE", __func__);

#ifndef AL6100_SPI_NOR
		err = mast_bulk_data_cmd_write_basic_code_shortlen(devdata, param,
				g_filp[BASIC_CODE]);
#endif
