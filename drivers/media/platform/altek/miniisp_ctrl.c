				...
				char *depth_qmerge_file_name)
{
	...
	u64 hdrPath, irp0Path, irp1Path, ppmapPath, depthPath;
	u8 cmd_param_buf[128];
	...
	depthPath  = (u64)depth_qmerge_file_name;
	...
	memcpy(&cmd_param_buf[64], &depthPath, 8);
...
/*
 ...
 B[0:0] InvRect bypass: set 1 for enable InvRect bypass. set 0 disable.
		(This function is incompatible with mini_isp_drv_write_depth_rectab_invrect,
		 Do not configure this bit if set mini_isp_drv_write_depth_rectab_invrect before)
 return Error code
*/
...
errcode mini_isp_drv_frame_sync_control(struct isp_cmd_frame_sync_control *frame_sync_control_param)
{
	/* Error Code*/
	errcode err = ERR_SUCCESS;
	/* Op code*/
	...
	memcpy(cmd_param_buf, frame_sync_control_param, sizeof(struct isp_cmd_frame_sync_control));
...
errcode mini_isp_drv_set_shot_mode(struct isp_cmd_set_shot_mode *set_shot_mode_param)
{
	/* Error Code*/
	errcode err = ERR_SUCCESS;
	/* Op code*/
	...
	memcpy(cmd_param_buf, set_shot_mode_param, sizeof(struct isp_cmd_set_shot_mode));
...
errcode mini_isp_drv_lighting_ctrl(struct isp_cmd_lighting_ctrl *lighting_ctrl)
{
	/* Error Code*/
	errcode err = ERR_SUCCESS;
	/* Op code*/
	...
	memcpy(cmd_param_buf, lighting_ctrl, sizeof(struct isp_cmd_lighting_ctrl));
 ...
 *\param en_update  [In], en_update = update short distance | update compensation
 ...
 *\param cycleLen  [In], en_update = update short distance | update global shift
 *\param depth_triggerBitField         [In], bit 0 : 1st frame, bit 1 : 2nd frame
 *\param depthoutput_triggerBitField   [In], bit 0 : 1st frame, bit 1 : 2nd frame
 *\return Error code
 */
errcode mini_isp_drv_cycle_trigger_depth_process(struct isp_cmd_cycle_trigger_depth_process *param)
{
	/* Error Code*/
	errcode err = ERR_SUCCESS;
	/* Op code*/
	...
	memset(cmd_param_buf, 0, T_SPI_CMD_LENGTH);
	memcpy(cmd_param_buf, param, sizeof(struct isp_cmd_cycle_trigger_depth_process));
